...
function Install-DreamSkinRuntimeEngine {
  ...
  $required = @(
    'assets\candy-background.png',
    'assets\dream-skin.css',
    'assets\candy-chick.svg',
    'assets\sprout-helper.svg',
    'assets\status-icons.svg',
    'assets\avatar-strawberry.svg',
    'assets\avatar-cloud.svg',
    'assets\avatar-star.svg',
    'assets\avatar-mint.svg',
    'assets\avatar-pudding.svg',
    'assets\avatar-cookie.svg',
...
function ConvertFrom-DreamSkinValidatedPackageRoot {
  param([Parameter(Mandatory = $true)][string]$PackageRoot)

  try {
    $packageRoot = [System.IO.Path]::GetFullPath($PackageRoot)
    $windowsApps = [System.IO.Path]::GetFullPath((Join-Path $env:ProgramFiles 'WindowsApps'))
    if (-not (Test-DreamSkinPathEqual -Left (Split-Path -Parent $packageRoot) -Right $windowsApps)) {
      return $null
    }

    $folderName = Split-Path -Leaf $packageRoot
    $folderMatch = [regex]::Match($folderName,
      '^OpenAI\.Codex_(?<version>\d+(?:\.\d+){3})_(?<arch>x64|x86|arm64|neutral)_(?<resource>[^_]*)_(?<publisher>[A-Za-z0-9]+)$',
      [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
    if (-not $folderMatch.Success) { return $null }

    $manifestPath = Join-Path $packageRoot 'AppxManifest.xml'
    $signaturePath = Join-Path $packageRoot 'AppxSignature.p7x'
    $executable = Join-Path $packageRoot 'app\ChatGPT.exe'
    ...
    if ("$($identity.Name)" -ine 'OpenAI.Codex' -or
      "$($identity.Version)" -ine $folderMatch.Groups['version'].Value -or
      "$($identity.ProcessorArchitecture)" -ine $folderMatch.Groups['arch'].Value) {
      return $null
    }
    ...
    if ($applications.Count -ne 1) { return $null }
    $applicationId = "$($applications[0].Id)"
    $publisherId = $folderMatch.Groups['publisher'].Value
    if ($publisherId -cnotmatch '^[A-Za-z0-9]{1,64}$' -or
      $applicationId -cnotmatch '^[A-Za-z0-9._-]{1,64}$') {
      return $null
    }
    ...
    if ($signature.Status -ne [System.Management.Automation.SignatureStatus]::Valid -or
      ...
      "$($signature.SignerCertificate.Subject)" -ine "$($identity.Publisher)" -or
      "$($signature.SignerCertificate.Issuer)" -inotmatch 'Microsoft Marketplace CA') {
      return $null
    }
...
function Get-DreamSkinRegisteredCodexInstalls {
  ...
  if ($installs.Count -eq 0) {
    $candidateRoots = @()
    if ($env:CODEX_DREAM_SKIN_PACKAGE_ROOT) {
      $candidateRoots += "$env:CODEX_DREAM_SKIN_PACKAGE_ROOT"
    }
    $candidateRoots += @(Get-Process -ErrorAction SilentlyContinue | ForEach-Object {
      try {
        if ($_.Path -like '*\WindowsApps\OpenAI.Codex_*\app\ChatGPT.exe') {
          Split-Path -Parent (Split-Path -Parent $_.Path)
        }
      } catch {}
    })
    foreach ($candidateRoot in @($candidateRoots | Select-Object -Unique)) {
      $install = ConvertFrom-DreamSkinValidatedPackageRoot -PackageRoot $candidateRoot
      if ($null -ne $install) { $installs += $install }
    }
  }
  return $installs
...
function Start-DreamSkinCodex {
  ...
  try {
    ...
    # Managed desktop sessions can expose the signed Store package while
    # denying its registration database to the current PowerShell process.
    # Delegate the already-validated executable to Explorer through a
    # short-lived shortcut so package ACLs and command-line arguments survive.
    $activationError = $_
    $shortcutPath = Join-Path $env:TEMP ("codex-dream-skin-launch-$PID-$([guid]::NewGuid().ToString('N')).lnk")
    try {
      $beforeIds = @(Get-DreamSkinCodexProcesses -Codex $Codex | ForEach-Object { [int]$_.ProcessId })
      $shell = New-Object -ComObject WScript.Shell
      $shortcut = $shell.CreateShortcut($shortcutPath)
      $shortcut.TargetPath = "$($Codex.Executable)"
      $shortcut.Arguments = $argumentLine
      $shortcut.WorkingDirectory = Split-Path -Parent "$($Codex.Executable)"
      $shortcut.Description = 'Launch the verified official Codex Store package'
      $shortcut.Save()
      ...
      do {
        ...
        $launched = @(Get-DreamSkinCodexProcesses -Codex $Codex | Where-Object {
          [int]$_.ProcessId -notin $beforeIds
        })
      } while ($launched.Count -eq 0 -and (Get-Date) -lt $deadline)
      if ($launched.Count -eq 0) { throw $activationError }
      return [int]$launched[0].ProcessId
    } finally {
...
function Get-DreamSkinCodexProcesses {
  ...
  # Some managed/sandboxed Windows sessions deny CIM while still exposing the
  # executable path through Get-Process. Preserve the same exact-path check.
  return @(Get-Process -Name 'ChatGPT' -ErrorAction SilentlyContinue | Where-Object {
      try { Test-DreamSkinPathEqual -Left $_.Path -Right $Codex.Executable } catch { $false }
    } | ForEach-Object {
      [pscustomobject]@{ ProcessId = $_.Id; ExecutablePath = $_.Path }
    })
}
...
function Stop-DreamSkinCodex {
  ...
  foreach ($item in $remaining) {
    ...
    if (-not $currentPath) {
      try { $currentPath = (Get-Process -Id $item.ProcessId -ErrorAction Stop).Path } catch { $currentPath = $null }
    }
