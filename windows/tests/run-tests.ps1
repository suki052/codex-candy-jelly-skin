...
try {
  ...
  if ($initialTheme.Theme.id -cne 'preset-codex-candy-jelly' -or
    $initialTheme.Theme.name -cne 'Codex 糖果果冻主题' -or
    ...
    throw 'Default Windows theme did not seed the Codex Candy Jelly contract.'
  }
  $preseededThemes = @(Get-DreamSkinSavedThemes -StateRoot $themeStateRoot)
  if ($preseededThemes.Count -ne 1 -or
    $preseededThemes[0].Id -cne 'preset-codex-candy-jelly' -or
    $preseededThemes[0].Name -cne 'Codex 糖果果冻主题') {
    throw 'Codex Candy Jelly was not preseeded in the Windows saved-theme menu.'
  }
  $updatedTheme = Set-DreamSkinActiveTheme -ImagePath (Join-Path $Root 'assets\candy-background.png') `
  ...
  Copy-Item -LiteralPath (Join-Path $Root 'assets\candy-background.png') `
    -Destination (Join-Path $outsideTheme 'candy-background.png')
  ...
  foreach ($requiredCss in @(
    ...
    '@font-face',
    'Dream Chill Round Gothic',
    '__RETRO_MENU_FONT_DATA__',
    '.dream-home-utility',
  ...
  foreach ($requiredMenuFontAsset in @(
    'assets\fonts\ChillRoundGothic_Medium.woff',
    'assets\fonts\OFL-ChillRoundGothic.txt'
  )) {
    if (-not (Test-Path -LiteralPath (Join-Path $Root $requiredMenuFontAsset) -PathType Leaf)) {
      throw "Bundled menu font asset is missing: $requiredMenuFontAsset"
    }
  }
  $injectorSource = Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\injector.mjs')
  foreach ($requiredMenuFontLoader in @(
    'ChillRoundGothic_Medium.woff',
    'data:font/woff;base64,',
    '__RETRO_MENU_FONT_DATA__'
  )) {
    if (-not $injectorSource.Contains($requiredMenuFontLoader)) {
      throw "Bundled menu font loader is missing: $requiredMenuFontLoader"
    }
  }
  ...
  foreach ($requiredGpuCompatibilityBehavior in @(
    '[switch]$DisableGpu',
    "`$arguments += '--disable-gpu'",
    'GPU compatibility mode requires an explicitly authorized Codex restart.'
  )) {
    if (-not $startSource.Contains($requiredGpuCompatibilityBehavior)) {
      throw "GPU compatibility launch behavior is missing: $requiredGpuCompatibilityBehavior"
    }
  }
  ...
  foreach ($requiredTaskRebindingBehavior in @(
    'shell.__retroTaskNodes',
    'taskNodesReplaced',
    'taskHitBindingMissing'
  )) {
    if (-not $rendererSource.Contains($requiredTaskRebindingBehavior)) {
      throw "Renderer task-node rebinding behavior is missing: $requiredTaskRebindingBehavior"
    }
  }
  foreach ($requiredFriendMenuBehavior in @(
    'data-retro-friend-flyout',
    'openFriendMenu',
    'closeFriendMenu',
    'data-retro-friend-command',
    'friendMenuAbort',
    'data-retro-friend-name="栗子"',
    '糖果好友 · 7/8'
  )) {
    if (-not $rendererSource.Contains($requiredFriendMenuBehavior)) {
      throw "Renderer friend-menu behavior is missing: $requiredFriendMenuBehavior"
    }
  }
  foreach ($requiredLeftRailBehavior in @(
    '糖果工作间',
    '今天也在认真敲代码',
    'data-retro-section-toggle',
    'retro-task-${taskState}',
    '正在处理 · 点击继续',
    '找找任务或文件…',
    'retroLeftWheelForwarding',
    'scheduleRetroTaskHitSync'
  )) {
    if (-not $rendererSource.Contains($requiredLeftRailBehavior)) {
      throw "Renderer left-rail behavior is missing: $requiredLeftRailBehavior"
    }
  }
  foreach ($requiredRightRailBehavior in @(
    'retro-profile-sky',
    'data-retro-friend-tabs',
    'data-retro-friend-panel="groups"',
    'selectFriendTab',
    'retro-friend-selected',
    '正在整理糖果'
  )) {
    if (-not $rendererSource.Contains($requiredRightRailBehavior)) {
      throw "Renderer right-rail behavior is missing: $requiredRightRailBehavior"
    }
  }
  foreach ($requiredStatusBarBehavior in @(
    'retro-status-state',
    'retro-status-safe',
    'retro-status-notice',
    'retro-status-flower',
    ...
    if (-not $rendererSource.Contains($requiredStatusBarBehavior)) {
      throw "Renderer candy status-bar behavior is missing: $requiredStatusBarBehavior"
    }
  }
  $themeSource = Read-DreamSkinUtf8File -Path (Join-Path $Root 'assets\dream-skin.css')
  foreach ($requiredFriendMenuStyle in @(
    '.retro-friend-flyout',
    '.retro-friend-menu-open',
    '.retro-friend-toast'
  )) {
    if (-not $themeSource.Contains($requiredFriendMenuStyle)) {
      throw "Theme friend-menu style is missing: $requiredFriendMenuStyle"
    }
  }
  foreach ($requiredLeftRailStyle in @(
    '.retro-left-title-copy',
    '.retro-shortcut-selected',
    '.retro-left-section-body.retro-section-collapsed',
    '.retro-task-icon-running',
    '.retro-left-search',
    'touch-action: pan-y',
    '[role="tooltip"]'
  )) {
    if (-not $themeSource.Contains($requiredLeftRailStyle)) {
      throw "Theme left-rail style is missing: $requiredLeftRailStyle"
    }
  }
  foreach ($requiredRightRailStyle in @(
    '.retro-profile-sky',
    '.retro-jelly-icon',
    '.retro-friend-tabs',
    '.retro-friend-tab-active',
    '.retro-friend-selected',
    '.retro-friend small.retro-online'
  )) {
    if (-not $themeSource.Contains($requiredRightRailStyle)) {
      throw "Theme right-rail style is missing: $requiredRightRailStyle"
    }
  }
  foreach ($requiredCandyFrameStyle in @(
    'Candy frame v1',
    '--candy-frame-pink',
    ...
    '.retro-status-bar',
    '.retro-status-state',
    '.retro-status-safe::before',
    '.retro-status-icon.icon-7',
    'html.codex-dream-skin .composer-surface-chrome'
  )) {
    if (-not $themeSource.Contains($requiredCandyFrameStyle)) {
      throw "Theme candy-frame style is missing: $requiredCandyFrameStyle"
    }
  }
  $injectorSource = Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\injector.mjs')
