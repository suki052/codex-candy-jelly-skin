...
function createFixture({
  ...
  const shellSidebar = {
    getBoundingClientRect() { return { left: 0, top: 36, width: 290, height: 784 }; },
  ...
  const createElement = (tagName) => {
    ...
    return {
      ...
      querySelector(selector) {
        if (selector === ".candy-shell") {
          return children.find((node) => String(node.className).split(/\s+/).includes("candy-shell")) ?? null;
        }
        return null;
      },
      querySelectorAll() { return []; },
